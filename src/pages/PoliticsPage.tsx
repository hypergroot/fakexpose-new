...
import { politicsArticles, PoliticsArticle } from '../data/politics';
...
const PoliticsPage = () => {
  ...
  const getRatingBadge = (rating: PoliticsArticle['rating']) => {
    switch (rating) {
      case 'true':
        return (
  ...
  const totalPages = Math.ceil(politicsArticles.length / articlesPerPage);
  const indexOfLastArticle = currentPage * articlesPerPage;
  const indexOfFirstArticle = indexOfLastArticle - articlesPerPage;
  const currentArticles = politicsArticles.slice(indexOfFirstArticle, indexOfLastArticle);

  return (
    <div className="py-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900">News & Politics</h1>
          <p className="mt-2 text-lg text-gray-600">
            Fact-checking political claims and news from around the world
          </p>
        </div>
